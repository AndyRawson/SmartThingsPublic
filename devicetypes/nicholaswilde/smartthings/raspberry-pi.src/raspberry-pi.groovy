...
metadata {
	...
	tiles {
        ...
        valueTile("cpuPercentage", "device.cpuPercentage", inactiveLabel: false) {
        	state "default", label:'${currentValue} ${name}', unit:"Percentage",
            backgroundColors:[
        ...
        main(["button", "temperature", "cpuPercentage", "memory" , "diskUsage"])
...
private getRPiData() {
    ...
    postAction(uri)
    //log.debug "Raspberry Pi postAction= ${uri}"
}
