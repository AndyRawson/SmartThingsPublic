...
preferences {
    section("Webhook URL"){
		input "url", "text", title: "Webhook URL", description: "Your webhook URL", required: false
	}

	section("Choose what events you want to trigger"){
    	input "myHub", "hub", title: "Hub", required: false, multiple: true
...
def subscribeToEvents() {
	...
	subscribe(myHub, "hub.event", logPing) 
}
...
def eventHandler(evt) {
	log.debug "Got Event ${evt.value}"
    log.debug "Hub: ${myHub.name} Supported Commands: ${myHub.getEvents}"
    location.hubs.each {
